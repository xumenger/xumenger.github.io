...
关于Hive 的数据仓库概念？其如何基于Hadoop 设计和实现的？Hive 与普通的关系型数据库比如MySQL 的异同？Hive 适用的场景？Spark SQL 如何对接Hive，以及编码的语法细节？等等这些内容可以作为研究Spark SQL 进阶的切入点，这里暂时就不深入讲解了，在以后具体用得到的时候再深究！
