...
![](../media/image/2020-12-29/gc-01.png)
...
结果测试环境数据倾斜，某一个用户有几百万条数据，程序没有使用游标，而是一次性加载，所以导致体现在程序层面是这个表对应的DAO 实体对象数量过多，然后程序循环几百万条数据处理的时间也很慢，所以这些DAO 实体对象在List 中一直被引用，Young GC 的时候无法使其被回收，反而会增加这些对象的年龄，最后这些对象还在被业务程序处理的时候，而在垃圾回收层面，已经被放到老年代了，导致触发Full GC
